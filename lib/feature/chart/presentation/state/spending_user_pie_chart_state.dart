...
class SpendingUserPieChartState extends StateNotifier<List<PieChartSectionData>> {
  ...
  // 当月の支出（ユーザー）算出
  void spendingUserChartCalc(DateTime date, List<QueryDocumentSnapshot<Map<String, dynamic>>> event, List<RoomMember> roomMember) {
    ...
    for (int i = 0; i < chartSourceData.length; i++) {
      int price = 0;
      price = setUserPriceFire(event, date, '支出', chartSourceData[i]['category']);
