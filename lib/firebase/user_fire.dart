...
Future<void> updateUserNameFire(name, code) async {
  ...
  await FirebaseFirestore.instance.collection('users').doc(code).collection('room').doc(uid).update({
    'userName': name,
  });
}
...
Future<void> updatePastEventUserName(code, oldName, newName) async {
  await FirebaseFirestore.instance.collection('users').doc(code).collection('events').where('paymentUser', isEqualTo: oldName).get()
      .then((QuerySnapshot snapshot) async {
    for (var doc in snapshot.docs) {
      await FirebaseFirestore.instance.collection('users').doc(code).collection('events').doc(doc.id).update({
        'paymentUser': newName,
      });
    }
  });
